...
type MsgHeader struct {
	...
	// Required: true
	// Enum: [search link resolve update unlink]
...
var msgHeaderTypeActionPropEnum []interface{}

func init() {
	var res []string
	if err := json.Unmarshal([]byte(`["search","link","resolve","update","unlink"]`), &res); err != nil {
		panic(err)
	}
	for _, v := range res {
		msgHeaderTypeActionPropEnum = append(msgHeaderTypeActionPropEnum, v)
	}
}

const (

	// MsgHeaderActionSearch captures enum value "search"
	MsgHeaderActionSearch string = "search"

	// MsgHeaderActionLink captures enum value "link"
	MsgHeaderActionLink string = "link"

	// MsgHeaderActionResolve captures enum value "resolve"
	MsgHeaderActionResolve string = "resolve"

	// MsgHeaderActionUpdate captures enum value "update"
	MsgHeaderActionUpdate string = "update"

	// MsgHeaderActionUnlink captures enum value "unlink"
	MsgHeaderActionUnlink string = "unlink"
)

// prop value enum
func (m *MsgHeader) validateActionEnum(path, location string, value string) error {
	if err := validate.EnumCase(path, location, value, msgHeaderTypeActionPropEnum, true); err != nil {
		return err
	}
	return nil
}
